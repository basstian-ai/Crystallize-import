...
async function createTopic(topic, context, parentId) {
    ...
    if (!createTopic) {
        ...
        context.emit(utils_1.EVENT_NAMES.ERROR, error);
        return null;
    }
    ...
    async function handleLevel(level, parentId) {
        ...
        try {
            ...
            onUpdate({
                ...
                message: `${(0, utils_1.getTranslation)(level.name, context.targetLanguage || context.defaultLanguage)}: ${!exists ? 'updated' : 'created'}`,
            });
            if (level.children) {
            ...
            onUpdate({
                error: {
                    code: 'OTHER',
                    message: 'Error occured on topic "' +
                        (0, utils_1.getTranslation)(level.name, context.targetLanguage || context.defaultLanguage) +
                        '"',
                },
            });
        }
