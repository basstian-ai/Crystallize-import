export function isWizardFullfilled(state) {
...
export function mapToReducerActions(dispatch) {
    return {
        setBoilerplate: (item) => dispatch({ type: 'SET_BOILERPLATE', item }),
        setTenant: (item) => dispatch({ type: 'SET_TENANT', item }),
        setBootstrapTenant: () => dispatch({ type: 'SET_BOOTSTRAP_TENANT' }),
        boilerplateDownloaded: () => dispatch({ type: 'BOILERPLATE_DOWNLOADED' }),
        recipesDone: (readme) => dispatch({ type: 'RECIPES_DONE', readme }),
        setCredentials: (credentials) => dispatch({ type: 'SET_CREDENTIALS', credentials }),
        changeTenant: (item) => dispatch({ type: 'CHANGE_TENANT', item }),
