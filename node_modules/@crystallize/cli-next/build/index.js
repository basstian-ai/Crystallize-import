...
const minVersion = 16;
if (major < minVersion) {
    output.error(styles.error(`You are running Node.js v${currentNodeVersion}. Crystallize CLI requires Node ${minVersion} or higher.`));
    process.exit(1);
}
const commands = {
...
run(cli.input, cli.flags).then((code) => {
    ...
    if (cli.flags.verbose) {
        consoleLogMemoryUsage((key) => styles.info(key));
    }
...
export async function run(args, flags) {
    ...
    try {
        const command = commands[args[0]] || help;
...
function consoleLogMemoryUsage(colorizedKeyFunc) {
    //@ts-ignore
